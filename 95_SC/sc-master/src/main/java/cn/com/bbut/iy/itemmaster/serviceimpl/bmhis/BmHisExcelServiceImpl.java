 ...
 */
@Slf4j
@Service(value = "bmHisExcelService")
public class BmHisExcelServiceImpl implements ExService {
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        BmHisExcelParam paramDTO = (BmHisExcelParam) param;
        Gson gson = new Gson();
        BmHisJsonParamDTO bmHisJsonParam = gson.fromJson(paramDTO.getParam().getSearchJson(),
                BmHisJsonParamDTO.class);
        session.setHeaderListener(new BmHisExcelHeaderListener(bmHisJsonParam));
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(bmHisJsonParam.getDiv())) {
                String temp = bmHisJsonParam.getDiv().substring(ConstantsDB.COMMON_TWO,
                        ConstantsDB.COMMON_THREE)
                        + STR_NINES;
                bmHisJsonParam.setDiv(temp.replaceAll(STR_NINE, quote));
            }
            if (StringUtils.isNotEmpty(bmHisJsonParam.getDpt())) {
                // 画面中dpt实际上是部，2位
                String temp = bmHisJsonParam.getDpt() + STR_NINE;
                bmHisJsonParam.setDpt(temp.replaceAll(STR_NINE, quote));
            }
            List<IyResourceDTO> paramResourceList = bmService.paramResourceList(
                    paramDTO.getRoleIds(), paramDTO.getPCode());
            bmHisJsonParam.setResources(paramResourceList);
            createExcelBody(sheet, curRow, bmHisJsonParam, page, rows);
     ...
     * @param curRow
     */
    private void createExcelBody(Sheet sheet, int curRow, BmHisJsonParamDTO bmHisJsonParam,
            int page, int rows) {
        // 数据开始坐标
        int limitStart = getLimitStart(page, rows);
        // 数据结束坐标
        int limitEnd = getLimitEnd(limitStart, rows);
        bmHisJsonParam.setLimitStart(limitStart);
        bmHisJsonParam.setLimitEnd(limitEnd);
        List<BmOutExcel> list = getBmOutExcelData(bmHisJsonParam);
        int no = 1;
        while (list != null && list.size() > 0) {
            ...
            for (BmOutExcel ls : list) {
                ...
                setCellValue(cell, ls.getUserId());
                cell = row.createCell(curCol++);
                cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_1));
                ...
                cell = row.createCell(curCol++);
                cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_1));
                setCellValue(cell, TimeUtil.formatSlantingDate(ls.getUpdateDate()));
            ...
            bmHisJsonParam.setLimitStart(limitStart);
            bmHisJsonParam.setLimitEnd(limitEnd);
            list = getBmOutExcelData(bmHisJsonParam);
        }
        int columnIndex = 0;
    ...
    private List<BmOutExcel> getBmOutExcelData(BmHisJsonParamDTO bmJsonParam) {
        List<BmOutExcel> restList = bmHisMapper.getBmOutExcelData(bmJsonParam);
    ...
    // 得到BM所属的对应中文翻译
    private String getNewFlg(String value) {
        if (value == null) {
            return "";
        }
        switch (value) {
    ...
    // 得到商品状态的对应中文翻译
    private String getCheckFlg(String value) {
        if (value == null) {
            return "";
        }
        switch (value) {
    ...
    // 得到商品状态的对应中文翻译
    private String getOpFlg(String value) {
        if (value == null) {
            return "";
        }
        switch (value) {
