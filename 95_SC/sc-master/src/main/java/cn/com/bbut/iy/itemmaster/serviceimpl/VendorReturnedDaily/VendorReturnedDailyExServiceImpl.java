...
import cn.com.bbut.iy.itemmaster.serviceimpl.vendorReceiptDaily.VendorReceiptDailyExHeaderListener;
...
@Slf4j
@Service(value = "vendorReturnedDailyExService")
public class VendorReturnedDailyExServiceImpl implements ExService {
    @Autowired
    private VendorReturnedDailyMapper mapper;
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        Gson gson = new Gson();
        VendorReturnedDailyParamDTO jsonParam = gson.fromJson(paramDTO.getParam(), VendorReturnedDailyParamDTO.class);
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new VendorReturnedDailyExHeaderListener(jsonParam));
     ...
     * @param curRow
     */
    private void createExcelBody(Sheet sheet, int curRow, VendorReturnedDailyParamDTO jsonParam) {
        // 查询数据
        List<VendorReturnedDailyDTO> _list = mapper.search(jsonParam);
        // 遍历数据
        int no = 1;
        for (VendorReturnedDailyDTO ls : _list) {
            int curCol = 0;
            ...
            setCellValue(cell, formatNum(ls.getReturnTotalQty()));
