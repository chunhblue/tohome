 ...
 */
@Slf4j
@Service(value = "storeTransferOutExService")
public class StoreTransferOutExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 特殊参数设置
        List<String> types = Arrays.asList("502");
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new StoreTransferOutExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, InventoryVouchersParamDTO jsonParam) {
        // 查询数据
        List<InventoryVouchersGridDTO> _list = inventoryVouchersMapper.selectByTypeCondition(jsonParam);
        // 遍历数据
        int no = 1;
