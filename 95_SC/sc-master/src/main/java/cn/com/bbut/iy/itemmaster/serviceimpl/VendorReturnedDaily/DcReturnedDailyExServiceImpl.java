...
@Slf4j
@Service(value = "dcReturnedDailyExService")
public class DcReturnedDailyExServiceImpl implements ExService {
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new DcReturnedDailyExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, VendorReturnedDailyParamDTO jsonParam) {
        // 查询数据
        List<VendorReturnedDailyDTO> _list = mapper.dcDailySearch(jsonParam);
        // 遍历数据
        int no = 1;
