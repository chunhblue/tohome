...
@Slf4j
@Service(value = "shopDiffExService")
public class ShopDiffExServiceLmpl  implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new ShopDiffExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, DifferenceListParam jsonParam) {
        // 查询数据
        List<DifferenceListResult> _list = mapper.selectShopDiffByCondition(jsonParam);
        // 遍历数据
        int no = 1;
