...
@Controller
@Slf4j
@RequestMapping(value = Constants.REQ_HEADER + "/dcReturnedDaily")
public class DcReturnedDailyController extends BaseAction  {
     ...
     * @return
     */
    @Permission(codes = { PermissionCode.CODE_SC_RETURN_DC_DAILY_LIST_VIEW})
    ...
    public ReturnDTO search(String jsonStr, HttpServletRequest request, HttpSession session) {
        ...
        Map<String,Object> result = vendorReturnedDailyService.deleteDcDailySearch(param);
    ...
    public List<AutoCompleteDTO> getAMList(String v, HttpServletRequest request, HttpSession session) {
        List<AutoCompleteDTO> result = vendorReturnedDailyService.getAMList("1",v);
        return result;
    }
    ...
    public List<AutoCompleteDTO> getOMList(String v,HttpServletRequest request, HttpSession session) {
        List<AutoCompleteDTO> result = vendorReturnedDailyService.getOMList("1",v);
        return result;
    }
    ...
    @Permission(codes = { PermissionCode.CODE_SC_RETURN_DC_DAILY_EXPORT })
    public String export(HttpServletRequest request, HttpSession session, String searchJson) {
        ...
        exParam.setPCode(PermissionCode.CODE_SC_RETURN_DC_DAILY_EXPORT);
        exParam.setExFileName(EXCEL_EXPORT_NAME);
        ExService service = Container.getBean("dcReturnedDailyExService", ExService.class);
     ...
     * @return
     */
    @Permission(codes = { PermissionCode.CODE_SC_RETURN_DC_DAILY_PRINT})
    ...
    public ReturnDTO printDate(String searchJson, HttpServletRequest request, HttpSession session) {
        ...
        param.setStores(stores);
        param.setFlg(false);
        List<VendorReturnedDailyDTO> result = vendorReturnedDailyService.deleteDcDailySearchPrint(param);
        return new ReturnDTO(true,"ok",result);
    }
