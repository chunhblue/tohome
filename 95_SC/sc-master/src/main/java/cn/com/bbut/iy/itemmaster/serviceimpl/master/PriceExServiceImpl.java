...
/**
 * Price By Day List Excel生成
 *
 */
@Slf4j
@Service(value = "priceExService")
public class PriceExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        Gson gson = new Gson();
        PriceByDayParamDTO jsonParam = gson.fromJson(paramDTO.getParam(), PriceByDayParamDTO.class);
        //设置资源
        ...
        // 获取业务日期
        if(StringUtils.isBlank(jsonParam.getEffectiveDate())){
            jsonParam.setEffectiveDate(cm9060Service.getValByKey("0000"));
        }
        // 生成文件标题信息对象
        session.setHeaderListener(new PriceExHeaderListener(jsonParam));
     ...
     * @param curRow
     */
    private void createExcelBody(Sheet sheet, int curRow, PriceByDayParamDTO jsonParam, String pcode) {
        // 查询数据
        List<PriceByDayDTO> _list = mapper.selectListByCondition(jsonParam);
        // 遍历数据
        int no = 1;
        for (PriceByDayDTO ls : _list) {
            int curCol = 0;
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_5));
            setCellValue(cell, ls.getSellingPrice());

            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_5));
            setCellValue(cell, ls.getBaseSalePrice());
