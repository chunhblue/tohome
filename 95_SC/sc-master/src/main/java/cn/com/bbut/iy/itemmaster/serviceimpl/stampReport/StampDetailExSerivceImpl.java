...
@Slf4j
@Service(value = "stampDetailExService")
public class StampDetailExSerivceImpl  implements ExService {
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new StampDetailExHeadSerivceImpl(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, StampSummaryParamReportDto jsonParam) {
        ...
        // 查询数据
        List<StampSummaryReportDto> _list = mapper.searchDetail(jsonParam);
        // 遍历数据
        int no = 1;
        for (StampSummaryReportDto ls : _list) {
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_1));
            setCellValue(cell, fmtDateAndTimeToStr(ls.getCreateYmdHms()));
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_4));
            setCellValue(cell, formatNum(ls.getBonusItemsQty()+""));

            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_4));
            setCellValue(cell, formatNum(ls.getAmountQty()+""));
