...
import static cn.com.bbut.iy.itemmaster.util.CommonUtils.STYPE_KEY_1;

/**
 * @ClassName cashierManagerExServiceImpl
 * @Description TODO
 ...
 */
@Slf4j
@Service(value = "cashierManagerExService")
public class CashierManagerExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        Gson gson = new Gson();
        SA0050ParamDto jsonParam=gson.fromJson(paramDTO.getParam(), SA0050ParamDto.class );
        // 导出不需要分页
        jsonParam.setFlg(false);
        jsonParam.setStores(paramDTO.getStores());
        // 生成文件标题信息对象
        session.setHeaderListener(new CashierExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow,  SA0050ParamDto param) {
        // 查询数据

        List<SA0050GridDto> _list = sa0050Mapper.getCashierListExcel(param);
        // 遍历数据
        int no = 1;
        for (SA0050GridDto ls : _list) {
            int curCol = 0;
...
//            setCellValue(cell, ls.getOfc());
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_3));
            setCellValue(cell, ls.getEffectiveStsName());
...
//            cell = row.createCell(curCol++);
//            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_3));
//            setCellValue(cell, ls.getDuty());

            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_3));
            setCellValue(cell, ls.getCashierEmail());

            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_3));
            setCellValue(cell, ls.getCashierLevel());
