 ...
 */
@Slf4j
@Service(value = "suspendSaleExService")
public class SuspendSaleExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new SuspendSaleExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, PriceDetailParamDto jsonParam) {
        // 查询数据
        List<PriceDetailGridDto> _list = suspendSaleMapper.getList(jsonParam);
        _list.forEach(priceDetailGridDto -> {
