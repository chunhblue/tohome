 ...
 */
@Slf4j
@Service(value = "storeTransferInExService")
public class StoreTransferInExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 特殊参数设置
        List<String> types = Arrays.asList("501");
        jsonParam.setVoucherType(null);
        jsonParam.setTypes(types);
        // 导出数据不需要分页
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new StoreTransferInExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, InventoryVouchersParamDTO jsonParam) {
        // 查询数据
        List<InventoryVouchersGridDTO> _list = inventoryVouchersMapper.selectByTypeInCondition(jsonParam);
        // 遍历数据
        int no = 1;
        for (InventoryVouchersGridDTO ls : _list) {
            ...
            // Tranfer Out Document No.
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_2));
            setCellValue(cell, ls.getVoucherNo1());
