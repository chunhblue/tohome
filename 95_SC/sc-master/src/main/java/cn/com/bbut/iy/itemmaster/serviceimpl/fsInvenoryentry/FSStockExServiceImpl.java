 ...
 */
@Slf4j
@Service(value = "fsStockExService")
public class FSStockExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new FSStockExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, PI0100ParamDTOC jsonParam) {
        // 查询数据
        List<PI0100DTOC> _list = fsInventoryplanMapper.search(jsonParam);
        // 遍历数据
        int no = 1;
