...
@Slf4j
@Secure
@RequestMapping(value = Constants.REQ_HEADER + "/customerCountByReport")
public class CustomerReceiptCountController extends BaseAction {
    @Autowired
    ...
    private final String EXCEL_EXPORT_NAME = "Store Card Payment Report For HCM.xlsx";
     ...
     public ModelAndView tolistView(HttpServletRequest request, HttpSession session,
        ...
        ModelAndView mv = new ModelAndView("customer_count_report/customerCountReport");
    ...
    public ReturnDTO getClassifiedSaleReportContent(String SearchJson, HttpServletRequest request, HttpSession session){
        CustomerCountParamDto param = null;
        if (SearchJson != null && !StringUtils.isEmpty(SearchJson)) {
            Gson gson = new Gson();
            param = gson.fromJson(SearchJson, CustomerCountParamDto.class);
        }
        if (param == null) {
        ...
        Map<String,Object> result = customerCountService.search(param);
    ...
    public String export(HttpServletRequest request, HttpSession session, String searchJson) {
        ...
        Gson gson = new Gson();
        CustomerCountParamDto param = gson.fromJson(searchJson, CustomerCountParamDto.class);
        // 获取当前角色店铺权限
        Collection<String> stores = getStores(session, param);
        if(stores.size() == 0){
            log.info(">>>>>>>>>>>>>>>>>>>>> get stores is null");
        ...
        ExService service = Container.getBean("customerCountExService", ExService.class);
        return ExportUtil.export(EXCEL_EXPORT_KEY, request, service, exParam);
    }

    private Collection<String> getStores(HttpSession session,CustomerCountParamDto param){
