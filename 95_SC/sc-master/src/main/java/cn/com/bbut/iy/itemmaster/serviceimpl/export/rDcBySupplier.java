 ...
 */
@Slf4j
@Service(value = "rDcBySupplier")
public class rDcBySupplier  implements ExService {
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new rWarehouseBySupplierExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, RWHListParam jsonParam) {
        ...
        List<RWHListResult> _list = returnWarehouseMapper.selectArticleBySupplier(jsonParam);
        // 遍历数据
        int no = 1;
