...
@Slf4j
//@Secure
@RequestMapping(value = Constants.REQ_HEADER + "/vendorReceiptDaily")
public class VendorReceiptDailyController extends BaseAction {
    ...
    private final String EXCEL_EXPORT_KEY = "EXCEL_RECEIVING_DAILY_SUPPLIER";
    private final String EXCEL_EXPORT_NAME = "Store Receiving Daily Report(To Supplier).xlsx";
     ...
     * @return
     */
    @Permission(codes = { PermissionCode.CODE_SC_VENDOR_DAILY_LIST_VIEW})
    ...
    public ModelAndView tolistView(HttpServletRequest request, HttpSession session,
        ...
        ModelAndView mv = new ModelAndView("vendorReceiptDaily/vendorReceiptDaily");
        mv.addObject("useMsg", "门店商品收货日报（Ｆrom 供应商）（含退货）");
    ...
    public ReturnDTO search(String jsonStr, HttpServletRequest request, HttpSession session) {
        ...
        Map<String,Object> result = vendorReceiptDailyService.deleteSearch(param);
        return new ReturnDTO(true,"ok",result);
    }
    ...
    public ReturnDTO printDate(String searchJson, HttpServletRequest request, HttpSession session) {
        ...
        param.setStores(stores);
        param.setFlg(false);
        List<VendorReceiptDailyDTO> result = (List<VendorReceiptDailyDTO>) vendorReceiptDailyService.deleteSearch(param).get("data");
        return new ReturnDTO(true,"ok",result);
    }
    @RequestMapping("/getAMList")
    @ResponseBody
    public List<AutoCompleteDTO> getAMList(String v,HttpServletRequest request, HttpSession session) {
        List<AutoCompleteDTO> result = vendorReceiptDailyService.getAMList("0",v);
        return result;
    }
    ...
    public List<AutoCompleteDTO> getOMList(String v,HttpServletRequest request, HttpSession session) {
        List<AutoCompleteDTO> result = vendorReceiptDailyService.getOMList("0",v);
        return result;
    }
    ...
    @Permission(codes = { PermissionCode.CODE_SC_VENDOR_DAILY_EXPORT })
    public String export(HttpServletRequest request, HttpSession session, String searchJson) {
        ...
        exParam.setPCode(PermissionCode.CODE_SC_VENDOR_DAILY_EXPORT);
        exParam.setExFileName(EXCEL_EXPORT_NAME);
        ExService service = Container.getBean("vendorReceiptDailyExService", ExService.class);
    ...
    public ModelAndView toPrintView(HttpServletRequest request, HttpSession session,String searchJson) {
        ...
        String ymd = nowDate.substring(0,8);
        String hms = nowDate.substring(8,14);
        log.debug("User:{} 进入 门店商品收货日报（Ｆrom 供应商）（含退货）打印画面", u.getUserId());
        Collection<Integer> roleIds = (Collection<Integer>) request.getSession().getAttribute(
                Constants.SESSION_ROLES);

        ModelAndView mv = new ModelAndView("vendorReceiptDaily/vendorReceiptDailyPrint");
        mv.addObject("useMsg", "门店商品收货日报（Ｆrom 供应商）（含退货）打印画面");
        mv.addObject("bsDate",Utils.getFormateDate(ymd) );
        mv.addObject("printTime", Utils.getFormateDate(ymd));
        mv.addObject("userName",u.getUserName());
