 ...
 */
@Slf4j
@Service(value = "vendorReceiptDailyExService")
public class VendorReceiptDailyExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new VendorReceiptDailyExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, VendorReceiptDailyParamDTO jsonParam) {
        // 创建前先删除临时表 2021/1/30
        mapper.deleteTempTable("tmp_order_article");
        // 查询数据
        List<VendorReceiptDailyDTO> _list = mapper.search(jsonParam);
        // 遍历数据
        int no = 1;
