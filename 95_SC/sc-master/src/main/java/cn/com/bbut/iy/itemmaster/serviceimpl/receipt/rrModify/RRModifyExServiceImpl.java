...
import cn.com.bbut.iy.itemmaster.dao.RRModifyMapper;
 ...
 */
@Slf4j
@Service(value = "rrModifyExService")
public class RRModifyExServiceImpl implements ExService {
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        Gson gson = new Gson();
        RRModifyParamDTO jsonParam = gson.fromJson(paramDTO.getParam(), RRModifyParamDTO.class);
        // 导出数据不需要分页
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new RRModifyExHeaderListener(jsonParam));
     ...
     * @param curRow
     */
    private void createExcelBody(Sheet sheet, int curRow, RRModifyParamDTO jsonParam) {
        // 查询数据
        List<RRModifyDTO> _list = rrMapper.selectListByCondition(jsonParam);
        // 遍历数据
        int no = 1;
        for (RRModifyDTO ls : _list) {
            int curCol = 0;
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_2));
            setCellValue(cell, getOrderType(ls.getOrderType()));
            ...
            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_2));
            setCellValue(cell, getStatus(ls.getReviewStatus()));

            cell = row.createCell(curCol++);
            cell.setCellStyle(MAP_STYLE.get(STYPE_KEY_2));
            setCellValue(cell, ls.getOrderRemark());
        ...
        sheet.setColumnWidth(columnIndex++, 36 * 256);
        sheet.setColumnWidth(columnIndex++, 20 * 256);
        sheet.setColumnWidth(columnIndex++, 20 * 256);
        sheet.setColumnWidth(columnIndex++, 20 * 256);
        sheet.setColumnWidth(columnIndex++, 20 * 256);
    ...
    private String getOrderType(String status) {
        ...
        switch (status) {
            case "07":
                return "[07] Receipt Document Correction";
            case "08":
                return "[08] Return Document Correction";
            default:
                return "";
        }
    }
