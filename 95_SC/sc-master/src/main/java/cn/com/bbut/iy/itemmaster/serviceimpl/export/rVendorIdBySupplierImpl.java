...
/**
 * @ClassName rVendorIdBySupplierImpl
 * @Description TODO
 * @Author Ldd
 ...
 */
@Slf4j
@Service(value = "rVendorIdBySupplier")
public class rVendorIdBySupplierImpl implements ExService {
    @Autowired
    ...
    public File getExcel(BaseExcelParam param) throws IOException {
        ...
        // 生成文件标题信息对象
        session.setHeaderListener(new rVendorIdBySuExHeaderListener(jsonParam));
    ...
    private void createExcelBody(Sheet sheet, int curRow, RVListParam jsonParam) {
        // 查询数据
        List<RVListResult> _list = returnVendorMapper.selectArticleBySupplier(jsonParam);
        // 遍历数据
        int no = 1;
