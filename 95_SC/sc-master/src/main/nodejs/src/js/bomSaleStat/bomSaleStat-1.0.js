...
define('groupSale', function () {
	...
	var but_event = function(){
		...
		m.print.on("click",function () {
			...
			var _param = "regionCd=" + regionCd + "&cityCd=" + cityCd + "&districtCd=" + districtCd +
				...
				districtName +"&storeName=" + storeName +"&saleStartDate=" + saleStartDate +
				"&saleEndDate=" + saleEndDate + "&articleId=" + articleId;
			var url = url_left +"/print?" + _param;
			window.open(encodeURI(url),"Group Sale Item Sales Report",'top=10,height=600,width=2000px,toolbar=no,menubar=yes,scrollbars=yes, resizable=yes,location=no, status=no,center:yes');
		});
		// 导出按钮事件
	...
	var verifySearch = function(){
		...
		if (!_storeCd) {
			_common.prompt("Please select a store first!",5,"error");/*请选择商店！*/
