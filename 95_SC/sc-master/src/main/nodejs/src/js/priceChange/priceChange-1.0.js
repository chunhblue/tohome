...
define('priceChange', function () {
 ...
 // 创建js对象
    ...
    function init(_common) {
    	createJqueryObj();
    	url_left =_common.config.surl+"/priceChange";
    ...
    var initOrganization = function () {
        ...
        a_region = $("#aRegion").myAutomatic({
            url: systemPath + "/suspendSaleEntry/selectListByLevel",
            ePageSize: 10,
            startCount: 0,
    ...
    var table_event = function(){
		$("#newPrice").blur(function () {
			...
			if (this.value<1) {
				_common.prompt("New price cannot be less than 1!",5,"error");
				let oldPrice = m.oldPrice.val()?m.oldPrice.val():'1';
				this.value=reThousands(oldPrice);
			}
			$("#newPrice").val(toThousands(this.value));
		});

		//光标进入，去除金额千分位，并去除小数后面多余的0
		$("#newPrice").focus(function(){
			$("#newPrice").val(reThousands(this.value));
		});
		...
		m.cancel.on("click",function () {
			_common.myConfirm("Are you sure you want to cancel",function(result){
				$("#articleId").css("border-color","#CCCCCC");
				$("#newPrice").css("border-color","#CCCCCC");
				if (result=="true"){
    ...
    var appendTrList = function () {
        ...
        _common.loading();
        $.myAjaxs({
            url: systemPath + "/suspendSaleEntry/getStoreListByDistrictCd",
            async: true,
            cache: false,
            type: "get",
            data: {
            ...
            success: function (result) {
                ...
                for (let i = 0; i < result.length; i++) {
                    ...
                    '<td tag="barcode" width="140" title="' + item.hidek + '" align="center" id="zgGridTtable_' + rowindex + '_tr_barcode" tdindex="zgGridTtable_barcode">' + item.hidek + '</td>' +
                    '</tr>';
                }
                // 等待框关闭
    ...
    var appendTr = function () {
        ...
        var tr = '<tr id="zgGridTtable_' + rowindex + '_tr" class="">' +
            ...
            '<td tag="barcode" width="140" title="' + barcode + '" align="center" id="zgGridTtable_barcode' + rowindex + '_tr_barcode" tdindex="zgGridTtable_barcode">' + barcode + '</td>' +
            '</tr>';
        tableGrid.append(tr);
    }
    //序号排序
    ...
    var but_event = function(){
    	...
    	m.submitBtn.on("click",function(){
            ...
            let changeId = m.changeId.val().trim();
            let oldPrice = m.oldPrice.val().trim();
            let newPrice = m.newPrice.val().trim();
            m.newPrice.css("border-color","#CCCCCC");
            m.articleId.css("border-color","#CCCCCC");
            if (articleId == null || articleId == "") {
                _common.prompt("Please select a item first!", 5, "error");/*请选择店铺*/
                m.articleId.css("border-color", "red");
                m.articleId.focus();
				return false;
			}
			if(accDate==null||accDate==""){
			...
			if (!newPrice) {
                _common.prompt("Please enter a new price！",5,"error");/*请输入新价格*/
                m.newPrice.css("border-color", "red");
                m.newPrice.focus();
                return false;
            }
	...
	var enableAll = function () {
		...
		m.newPrice.prop("disabled",false);
    ...
    var initTable1 = function(){
		tableGrid = $("#zgGridTtable").zgGrid({
    		...
    		colModel:[
                ...
                {name: "regionCd", type: "text", text: "right", width: "130", ishide: false, css: ""},
                ...
                {name: "barcode", type: "text", text: "right", width: "140", ishide: false, css: ""}
	          ],//列内容
	          width:"max",//宽度自动
	          isPage:false,//是否需要分页
	          isCheckbox:false,
...
var _index = require('priceChange');
