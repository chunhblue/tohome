...
define('staffAttendanceReport', function () {
    var self = {};
    var url_left = "",
        systemPath = "",
        reThousands = null,
    ...
    function init(_common) {
        createJqueryObj();
        url_left = _common.config.surl + "/staffAttendanceReport";
                ...
                if (result.success) {
                    ...
                    for (let i = 0; i < dataList.length; i++) {
                        ...
                        let tempTrHtml =
                            ...
                            '<td title="'+fmtIntDate(item.dateIn)+'" style="text-align: center">'+fmtIntDate(item.dateIn)+'</td>' +
                            '<td title="'+isEmpty(item.staffCode)+'" style="text-align: right">'+isEmpty(item.staffCode)+'</td>' +
                            '<td title="'+isEmpty(item.staffName)+'" style="text-align: left">'+isEmpty(item.staffName)+'</td>' +
                            '<td title="'+fmtIntDate(item.dateIn1)+'" style="text-align: center">'+fmtIntDate(item.dateIn1)+'</td>' +
                            '<td title="'+fmtIntTime(item.timeIn)+'" style="text-align: center">'+fmtIntTime(item.timeIn)+'</td>' +
                            '<td title="'+fmtIntDate(item.dateOut)+'" style="text-align: center">'+fmtIntDate(item.dateOut)+'</td>' +
                            '<td title="'+fmtIntTime(item.timeOut)+'" style="text-align: center">'+fmtIntTime(item.timeOut)+'</td>' +
                            '<td title="'+isEmpty(item.hourWorked)+'" style="text-align: center">'+isEmpty(item.hourWorked)+'</td>' +
                            '<td title="'+isEmpty(item.shift)+'" style="text-align: left">'+isEmpty(item.shift)+'</td>' +
                            '</tr>';
    ...
    // 格式化数字类型的时分 175829 --> 17:58
    function fmtIntTime(date){
        var res = "";
        res = date.substring(0,2)+":"+date.substring(2,4);
        return res;
    }
...
var _index = require('staffAttendanceReport');
