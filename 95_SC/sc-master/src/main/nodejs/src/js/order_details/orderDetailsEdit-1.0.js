...
define('orderDetailsEdit', function () {
    var self = {};
    var url_left = "",
		systemPath = "",
	    paramGrid = null,
    ...
    var m = {
		...
		vendor:null,
		discountRate:null, // 订单折扣率
		vendorEmail:null,
		minOrderQty:null,//供应商最低订货数量
		minOrderAmtTax:null,//供应商最低订货金额
		search_vendor_input:null,
		orderRemark:null,//备注
		ca_date:null,//提交时间
		ca_user:null,//提交用户
		md_date:null,//修改时间
 ...
 // 创建js对象
    ...
    var table_event = function(){
		$("#orderQty").blur(function () {
			...
			if(reg.test(orderQty)&&orderPrice!=null&&orderPrice!=''){
		...
		$("#addOrderDetails").on("click", function () {
			if(verifySearch()){
				...
				m.item_input_orderNochargeQty.attr("disabled",false);
		...
		$("#updateOrderDetails").on("click",function(){
			if(verifySearch()){
				...
				itemClear();
				var str = "&storeCd="+$("#aStore").attr("k")+"&vendorId="+$("#a_vendor").attr("k")+"&orderDate="+subfmtDate(m.od_date.val());
		...
		$("#viewOrderDetails").on("click",function(){
			if(verifySearch()){
				...
				m.item_input_orderNochargeQty.attr("disabled",true);
		...
		m.auto_import_item.on("click",function(){
			...
			if(vendorId==null||vendorId==''){
				_common.prompt("Please select the vendor!",3,"info");
				return false;
			}
			_common.myConfirm("Vendor Code cannot be changed after click 'Confirm', are you sure to confirm?",function(result) {
				if (result == "true"&&verifySearch()) {
					paramGrid = "orderDate=" + subfmtDate(m.od_date.val())+
						"&storeCd="+$("#aStore").attr("k")+
						"&vendorId="+$("#a_vendor").attr("k");
					tableGrid.setting("url", url_left + "/getItemsByVendorAuto");
					tableGrid.setting("param", paramGrid);
					tableGrid.loadData(null);
					m.auto_import_item.attr("disabled","disabled").css("pointer-events","none");
					$("#a_vendor").attr("disabled","disabled");
					m.vendor_clear.hide();
					m.vendor_refresh.hide();
			...
			if(back!='disabled'&&flg!='0'){
				_common.myConfirm("Crrent change is not submitted yet,are you sure to exit?",function(result) {
					if (result == "true") {
						...
						top.location = systemPath + "/directSupplierOrder";
					}
				})
			}else{
				_common.updateBackFlg(KEY);
		...
		m.affirm.on("click",function(){
			...
			if($("#zgGridTtable>.zgGrid-tbody tr").length>0){
				let _purchaseVatCd = $("#zgGridTtable>.zgGrid-tbody tr:eq(0)").find('td[tag=purchaseVatCd]').text();
				if(m.purchaseVatCd.val()!=_purchaseVatCd){
					//该商品税率不同，请重新选择商品
				...
				if(result=="true"){
					if(flg=='add'){
						...
						var tr = '<tr id="zgGridTtable_'+rowindex+'_tr" class="">' +
							...
							'<td tag="psd" width="230" title="'+m.item_input_psd.val()+'" align="right" id="zgGridTtable_'+rowindex+'_tr_psd" tdindex="zgGridTtable_psd">'+m.item_input_psd.val()+'</td>' +
							...
							'<td tag="minOrderQty" width="160" title="'+m.item_input_minOrderQty.val()+'" align="right" id="zgGridTtable_'+rowindex+'_tr_minOrderQty" tdindex="zgGridTtable_minOrderQty">'+m.item_input_minOrderQty.val()+'</td>' +
							'<td tag="orderBatchQty" hidden width="160" title="'+m.item_input_orderBatchQty.val()+'" align="right" id="zgGridTtable_'+rowindex+'_tr_orderBatchQty" tdindex="zgGridTtable_orderBatchQty">'+m.item_input_orderBatchQty.val()+'</td>' +
							...
							'<td tag="orderNochargeQty" width="200" title="'+m.item_input_orderNochargeQty.val()+'" align="right" id="zgGridTtable_'+rowindex+'_tr_orderNochargeQty" tdindex="zgGridTtable_orderNochargeQty">'+m.item_input_orderNochargeQty.val()+'</td>' +
		...
		m.returnsSubmitBut.on("click",function(){
			...
			//最低订货金额
			var minOrderAmtTax = reThousands(m.minOrderAmtTax.val());//最小订货金额
			if(minOrderAmtTax!=null&&minOrderAmtTax!=''){//没有最小订货金额则不判断
				var totalAmt = parseFloat(reThousands($("#order_total_amt").val()));
				var minOrderAmt = parseFloat(minOrderAmtTax);
				if(!isNaN(totalAmt)&&!isNaN(minOrderAmt)){
					if(totalAmt<minOrderAmt){
						let differenceAmount = minOrderAmt-totalAmt;//差异金额
						_common.prompt("The Total Amount must not be less than the Min. Order Amount,difference amount: "+toThousands(differenceAmount)+"!",5,"info");// 总金额不得小于最低订货金额
						return false;
					}
				}else{
					_common.prompt("Min. Order Amount or Total Amount error!",5,"info");// 最低订货金额或者总金额出错
					return false;
				}
			}
			/*
			//供应商最低订货量
			var minOrderQty = reThousands(m.minOrderQty.val());
			if(minOrderQty!=null&&minOrderQty!=''){//没有最小订货量则不判断
				var totalQty = parseFloat(reThousands($("#order_total_qty").val()));
				var _minOrderQty = parseFloat(minOrderQty);
				if(!isNaN(totalQty)&&!isNaN(_minOrderQty)){
					if(totalQty<_minOrderQty){
						let differenceQty = _minOrderQty-totalQty;//差异数量
						_common.prompt("The Total Qty must not be less than the Min. Order Qty,difference quantity: "+toThousands(differenceQty)+"!",5,"info");// 总数量不得小于最低订货数量
						return false;
					}
				}else{
					_common.prompt("Min. Order Amount or Total Amount error!",5,"info");// 最低订货金额或者总金额出错
			...
			*/
			...
			$("#zgGridTtable>.zgGrid-tbody tr").each(function () {
				var orderItem = {
					...
					orderPrice: reThousands($(this).find('td[tag=orderPrice]').text()),//订货单价（不含税）
					orderAmtNotax: reThousands($(this).find('td[tag=orderTotalAmt]').text()),//订货价格（不含税）
    ...
    var setDialogValue = function () {
		var cols = tableGrid.getSelectColValue(selectTrTemp,"barcode,articleId,articleName,unitId,unitName,spec,converter,orderPrice,orderBatchQty,salePrice,orderTotalAmt,psd,minDisplayQty,minOrderQty,autoOrderQty,orderQty,orderNochargeQty,purchaseVatCd");
	...
	var verifySearch = function(){
		...
		if(m.od_date.val()==null||m.od_date.val()==""){
			_common.prompt("Please enter the order date!",5,"error"); // 请输入订货日期
		...
		var vendorId = $("#a_vendor").attr("k");
		if(vendorId==null||vendorId==""){
			_common.prompt("Please select vendor!",5,"error"); // 请选择供应商
			$("#a_vendor").focus();
			$("#a_vendor").css("border-color", "red");
			return false;
		}else {
			$("#a_vendor").css("border-color", "#CCC");
		}
		var str = "&storeCd=" + storeCd+"&vendorId="+vendorId+"&orderDate="+subfmtDate(m.od_date.val());
	...
	var initView = function () {
		if (m.orderId.val() != null && m.orderId.val() != "") {
			$.myAjaxs({
				url:url_left+"/getOrderVendorInfo",
				async:true,
				cache:false,
				type :"post",
				...
				success:function(result){
					if(result.success){
						...
						// $.myAutomatic.setValueTemp(a_region,result.data.storeCd,result.data.regionName);//店铺赋值
						// $.myAutomatic.setValueTemp(a_city,result.data.storeCd,result.data.cityName);//店铺赋值
						// $.myAutomatic.setValueTemp(a_district,result.data.storeCd,result.data.districtName);//店铺赋值
						$.myAutomatic.setValueTemp(a_store,result.data.storeCd,result.data.storeName);
						$.myAutomatic.setValueTemp(a_vendor,result.data.vendorId,result.data.vendorId+" "+result.data.vendorName);//供应商赋值
						m.discountRate.val(result.data.discountRate); // 折扣率(百分比)
						m.minOrderAmtTax.val(toThousands(result.data.minOrderAmtTax));//供应商最低订货金额
						m.minOrderQty.val(toThousands(result.data.minOrderQty));//供应商最低订货量
						m.vendorEmail.val(result.data.vendorEmail);//供应商邮箱
						...
						tableGrid.setting("url", url_left + "/getItemsByVendor");
						tableGrid.setting("param", paramGrid);
						tableGrid.loadData(null);
					}else{
    ...
    var but_event = function(){
        ...
        m.search.on("click",function(){
			...
			if(vendorId==null||vendorId==''){
				...
				$.myAjaxs({
					url:systemPath+"/ma2000/getVendorById",
					async:true,
					cache:false,
					type :"post",
					...
					success:function(result){
						if(result.success){
							...
							$('#vendorEmail').val(result.data.vendorEmail);
							$('#vendorAddress1').val(result.data.vendorAddress1);
							$('#vendorAddress2').val(result.data.vendorAddress2);
							$('#vendorTelNo').val(result.data.vendorTelNo);
							$('#vendorFaxNo').val(result.data.vendorFaxNo);
							$('#orderSendMethod').val(result.data.orderSendMethod);
							$('#shipmentFlg').val(result.data.shipmentFlg);
							$('#cutoffTime').val(result.data.cutoffTime);
							$('#orderAddress1').val(result.data.orderAddress1);
							$('#orderAddress2').val(result.data.orderAddress2);
							$('#orderTelNo').val(result.data.orderTelNo);
							$('#orderFaxNo').val(result.data.orderFaxNo);
							$('#orderAdminName').val(result.data.orderAdminName);
							$('#supplyPurchaseRate').val(result.data.supplyPurchaseRate);
							$('#badDiscountRate').val(result.data.badDiscountRate);
							//显示弹出框
							$("#vendor_dialog").modal("show");
						}else{
							_common.prompt(result.message,5,"error"); // 供应商数据加载失败
						}
					},
					complete:_common.myAjaxComplete
				});
		...
		a_store = $("#aStore").myAutomatic({
			...
			selectEleClick: function (thisObj) {
				...
				if (thisObj.attr("k") != null && thisObj.attr("k") != "") {
					$("#a_vendor").attr("disabled",false);
					$("#vendor_refresh").show();
					$("#vendor_clear").show();
				}
				// 替换子级查询参数
				var str = "&storeCd=" + thisObj.attr("k");
				$.myAutomatic.replaceParam(a_vendor, str);
				$(".zgGrid-tbody").empty();
			}
		});
		...
		$.myAjaxs({
			...
			success:function(re){
				if(re.success){
					...
					// 替换供应商查询参数
					var str_1 = "&storeCd=" + obj.storeCd;
					$.myAutomatic.replaceParam(a_vendor, str_1);
				}
			}
		});
		//供应商
		a_vendor = $("#a_vendor").myAutomatic({
			url: systemPath + "/ma2000/getVendorAuto",
			ePageSize: 10,
			startCount: 0,
			cleanInput: function (thisObj) {
				$(".zgGrid-tbody").empty();
				m.minOrderAmtTax.val("");
				m.vendorEmail.val("");
				m.minOrderQty.val("");
			},
			selectEleClick: function (thisObj) {
				...
				if (thisObj.attr("k") != null && thisObj.attr("k") != "") {
					$.myAjaxs({
						url:url_left+"/getVendorInfo",
						async:true,
						cache:false,
						type :"post",
						data :"vendorId="+thisObj.attr("k")+"&storeCd="+$("#aStore").attr("k"),
						dataType:"json",
						success:function(result){
							if(result.success){
								m.minOrderAmtTax.val(toThousands(result.data.minOrderAmtTax));
								m.minOrderQty.val(toThousands(result.data.minOrderQty));
								m.vendorEmail.val(result.data.vendorEmail);
							}
						},
						error : function(e){
							_common.prompt("Failed to load supplier data",5,"error"); // 供应商数据加载失败
						},
						complete:_common.myAjaxComplete
					});
				}
	...
	var initTable1 = function(){
		tableGrid = $("#zgGridTtable").zgGrid({
			title:"Order List",
			param:paramGrid,
			colNames:["Item Barcode","Item Code","Item Name","Unit Id","UOM","Specification","Converter","Purchasing Price",
				"Price","PSD","Display on Shelf","Min. Order Qty","Incremental Quantity",
				"Recommended Order Qty","Order Quantity","Order Amount","Free Order Quantity","Order Total Quantity","Purchase Vat Cd"],
			colModel:[
				...
				{name:"totalQty",type:"text",text:"right",width:"200",ishide:false,css:"",getCustomValue:getThousands},
...
var _index = require('orderDetailsEdit');
