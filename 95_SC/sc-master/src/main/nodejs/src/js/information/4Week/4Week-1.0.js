...
define('fourWeekOperation', function () {
    var self = {};
    var url_left = "",
	    paramGrid = null,
	    selectTrTemp = null,
    ...
    function init(_common) {
		createJqueryObj();
		url_left = _common.config.surl + "/4WeekOperation";
		//首先验证当前操作人的权限是否混乱，
		if (m.use.val() == "0") {
			but_event();
		} else {
	...
	var but_event = function(){
		...
		//绑定事件
		$("#businessDate").datetimepicker().on('changeDate', function(ev){
			if(ev.date == null || ev.date == ''){
				m.businessDate.val('');
			}
			var _date = ev.date.valueOf();
			$.myAjaxs({
				url:url_left+"/getWeeks",
				type :"get",
				data :{'date': _date},
				success:function(result){
					m.currentWeek.val(result);
				},
				error : function(e){
					m.currentWeek.val("");
				}
	...
	var initPageBytype = function(flgType){
		switch(flgType) {
			case "1":
				...
				initTable2();//列表初始化-部门级、大类级、小类级
				$("#div_table2").hide();
				initTable3();//列表初始化-大类级
				$("#div_table3").hide();
				break;
			case "2":
				initTable2();//列表初始化
	...
	var initTable1 = function(){
		var data = '[{"sc1":"本周","sc2":"1000000.00","sc3":"50.00","sc4":"1.00","sc5":"1.00","sc6":"1.00","sc7":"1.00","sc8":"1.00","sc9":"1","sc10":"100000.00","sc11":"1.00"}]';
		tableGrid1 = $("#zgGridTable1").zgGrid({
			title:"Query Result",//查询结果
			param:paramGrid,
			localSort: true,
			colNames:["Week","Sales Amount","GP Ratio(%)","Promotion Amount","Promotion Ratio(%)","Budget Achievement Ratio(%)",
				"Sales Growth Ratio(%)","Average Daily Customer Quantity","Customer Deal Amount","Purchasing Amount","Adjustment Amount"],
			colModel:[
				...
				{name:"sc5",type:"text",text:"center",width:"140",ishide:false,css:"",getCustomValue:null},
				{name:"sc6",type:"text",text:"center",width:"200",ishide:false,css:"",getCustomValue:null},
				{name:"sc7",type:"text",text:"center",width:"150",ishide:false,css:"",getCustomValue:null},
				{name:"sc8",type:"text",text:"center",width:"230",ishide:false,css:"",getCustomValue:null},
				{name:"sc9",type:"text",text:"center",width:"170",ishide:false,css:"",getCustomValue:null},
				{name:"sc10",type:"text",text:"center",width:"140",ishide:false,css:"",getCustomValue:null},
	...
	var initTable2 = function(){
		...
		tableGrid2 = $("#zgGridTable2").zgGrid({
			...
			param:paramGrid,
			localSort: true,
			colNames:["Week","Sales Amount","GP Ratio(%)","Propotion in Store(%)","Promotion Amount","Promotion Ratio(%)",
				"Sales Growth Ratio(%)","Purchasing Amount","Adjustment Amount"],
			colModel:[
				{name:"sc1",type:"text",text:"center",width:"130",ishide:false,css:""},
				{name:"sc2",type:"text",text:"center",width:"130",ishide:false,css:""},
				{name:"sc3",type:"text",text:"center",width:"130",ishide:false,css:""},
	...
	var initTable3 = function(){
		...
		tableGrid3 = $("#zgGridTable3").zgGrid({
			...
			param:paramGrid,
			localSort: true,
			colNames:["Week","Sales Quantity","Sales Amount","GP Ratio(%)","Promotion Amount","Promotion Ratio(%)",
				"Purchasing Quantity","Purchasing Amount","Adjustment Quantity","Adjustment Amount"],
			colModel:[
				{name:"sc1",type:"text",text:"center",width:"130",ishide:false,css:""},
				{name:"sc2",type:"text",text:"center",width:"130",ishide:false,css:""},
				{name:"sc3",type:"text",text:"center",width:"130",ishide:false,css:""},
...
var _index = require('fourWeekOperation');
