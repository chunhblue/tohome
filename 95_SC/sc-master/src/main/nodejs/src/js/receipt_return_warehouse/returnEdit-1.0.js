...
define('receiptReturnWarehouseEdit', function () {
	var self = {};
	var url_left = "",
		systemPath = "",
		paramGrid = null,
		...
		tempTrOrderNoChargeQty = null,//选中的退货搭赠量
	    tempTrReceiveNoChargeQty = null,// Actually Return Free
		common=null;
					...
					$.myAjaxs({
						...
						success:function(result){
							if(result.success){
								...
								m.store_name.val(data.storeName);
								m.dc_id.val(data.dcId);
								m.dc_name.val(data.dcName);
							}else{
								m.store_cd.val('');
								m.store_name.val('');
								m.dc_id.val('');
		...
		$.myAjaxs({
			...
			success:function(result){
				if(result.success){
					...
					m.store_name.val(result.data.storeName);
					m.dc_id.val(result.data.deliveryCenterId);
					m.dc_name.val(result.data.deliveryCenterId+' '+result.data.deliveryCenterName);
	...
	var table_event = function(){
		...
		m.submit.on("click", function () {
			...
			if(receiveNoChargeQty>orderNoChargeQty) {//不得高于退货量
				_common.prompt("Actual Return Free Qty not be greater than Returning Free Qty!",3,"info");
				m.receiveNoChargeQty.focus();
				$("#receiveNoChargeQty").css("border-color","red");
				return false;
			}else {
	...
	var clearTempTr = function(){
		...
		tempTrReasonId = null;//选中的退货原因
		tempTrReasonName = null;//选中的退货原因
		tempTrReceiveNoChargeQty=null;
		tempTrOrderNoChargeQty=null;
	}
	...
	var trClick_table1 = function(){
		//选中时先清空临时值
		clearTempTr();
		var cols = tableGrid.getSelectColValue(selectTrTemp,"barcode,articleId,articleName,unitId,unitName,spec,orderQty,orderNoChargeQty,orderPrice,orderAmount,reasonId,reasonName,receiveQty,receiveNoChargeQty");
...
var _index = require('receiptReturnWarehouseEdit');
