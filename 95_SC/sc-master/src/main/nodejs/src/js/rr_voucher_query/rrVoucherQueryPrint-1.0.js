...
define('receiptPrint', function () {
    ...
    function init(_common) {
        createJqueryObj();
        url_left = _common.config.surl+"/rrVoucherQuery/print";
                ...
                if(result.success){
                    ...
                    for (let i = 0; i < dataList.length; i++) {
                        ...
                        let tempTrHtml = '<tr style="text-align: center;">' +
                            ...
                            '<td style="text-align: left">'+isEmpty(item.orgOrderId)+'</td>' +
                            '<td style="text-align: center">'+dateFmt(item.orgOrderDate)+'</td>' +
                            '<td style="text-align: left">'+getTypeName(item.orderType)+'</td>' +
                            '<td style="text-align: right">'+isEmpty(item.storeCd)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.storeName)+'</td>' +

                            // '<td style="text-align: right">'+isEmpty(item.ofc)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.ofcName)+'</td>' +
                            // '<td style="text-align: right">'+isEmpty(item.om)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.omName)+'</td>' +
                            // '<td style="text-align: left">'+isEmpty(item.oc)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.ocName)+'</td>' +

                            '<td style="text-align: right">'+isEmpty(item.warehouseCd)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.warehouseName)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.orderMethodName)+'</td>' +
                            '<td style="text-align: right">'+fmtRate(item.taxRate)+'</td>' +
                            '<td style="text-align: left">'+isEmpty(item.reviewStsText)+'</td>' +
                            '</tr>';
