...
define('customerCountReport', function () {
    var self = {};
    var url_left = "",
        systemPath = "",
        reThousands = null,
    ...
    function init(_common) {
        createJqueryObj();
        url_left = _common.config.surl + "/customerCountByReport";
                ...
                if (result.success) {
                    ...
                    for (let i = 0; i < dataList.length; i++) {
                        ...
                        let tempTrHtml =
                            ...
                            '<td title="'+item.dateTime+'" style="text-align: center">'+item.dateTime+'</td>' +
                            '<td title="'+fmtIntNum(item.k1bill19k)+'" style="text-align: center">'+fmtIntNum(item.k1bill19k)+'</td>' +
                            '<td title="'+fmtIntNum(item.k19bill29k)+'" style="text-align: center">'+fmtIntNum(item.k19bill29k)+'</td>' +
                            '<td title="'+fmtIntNum(item.k29bill39k)+'" style="text-align: center">'+fmtIntNum(item.k29bill39k)+'</td>' +
                            '<td title="'+fmtIntNum(item.k39bill49k)+'" style="text-align: center">'+fmtIntNum(item.k39bill49k)+'</td>' +
                            '<td title="'+fmtIntNum(item.bill49k)+'" style="text-align: center">'+fmtIntNum(item.bill49k)+'</td>' +
                            '<td title="'+fmtIntNum(item.total)+'" style="text-align: center">'+fmtIntNum(item.total)+'</td>' +
                            '</tr>';
    ...
    var verifySearch = function(){
        ...
        if(fmtDate(m.startDate.val())>fmtDate(m.endDate.val())){
            $("#endDate").focus();
...
var _index = require('customerCountReport');
