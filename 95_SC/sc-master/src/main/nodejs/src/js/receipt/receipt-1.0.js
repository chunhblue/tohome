...
define('receipt', function () {
	...
	var initParam = function () {
		...
		$.myAutomatic.setValueTemp(warehouseCd,obj.warehouseCd,obj.warehouseName);
		// 设置组织架构回显
	...
	var saveParamToSession = function () {
		...
		obj.warehouseName=$('#warehouseCd').attr('v');
	...
	var table_event = function(){
		...
		$("#update").on("click",function(){
			...
			let cols = tableGrid.getSelectColValue(_list[0],"receiveId,reveiveStatus,storeCd,orderId,orderSts,multipleFlg,reviewSts");
			if(cols["orderSts"]=='04'){
    ...
    var but_event = function(){
		...
		//清空日期
		m.clear_ck_date.on("click",function(){
			m.ck_start_date.val("");
			m.ck_end_date.val("");
		});
		//开始日
		m.ck_start_date.datetimepicker({
			language:'en',
			format: 'dd/mm/yyyy',
			maxView:4,
			startView:2,
	...
	var verifySearch = function(){
		...
		if(m.ck_start_date.val()!="" && m.ck_end_date.val()!=""){
			let _ckStartDate = new Date(fmtDate($("#ck_start_date").val())).getTime();
			if(_common.judgeValidDate($("#ck_start_date").val())){
				_common.prompt("Please enter a valid date!",3,"info");
				$("#ck_start_date").focus();
				return false;
			}
			let _ckEndDate = new Date(fmtDate($("#ck_end_date").val())).getTime();
			if(_common.judgeValidDate($("#ck_end_date").val())){
				_common.prompt("Please enter a valid date!",3,"info");
				$("#ck_end_date").focus();
				return false;
			}
			if(_ckStartDate>_ckEndDate){
				_common.prompt("The start date cannot be greater than the end date!",3,"info");/*开始时间不能大于结束时间*/
				$("#ck_end_date").focus();
				return false;
			}
			let ckdifValue = parseInt(Math.abs((_ckEndDate-_ckStartDate)/(1000*3600*24)));
			if(ckdifValue >62){
				_common.prompt("Query Period cannot exceed 62 days!",3,"info"); // 日期期间取值范围不能大于62天
				$("#ck_end_date").focus();
				return false;
    ...
    var setParamJson = function(){
		...
		// 出库日期
		var _deliveryStartDate = fmtStringDate(m.ck_start_date.val())||null;
		var _deliveryEndDate = fmtStringDate(m.ck_end_date.val())||null;
		...
		// 创建请求字符串
 		var searchJsonStr ={
			...
			receiveId:m.receiveId.val().trim(),
			warehouseCd:$("#warehouseCd").attr("k"),
			voucherStatus:m.dj_status.val().trim(),
	...
	var initTable = function(){
		tableGrid = $("#zgGridTable").zgGrid({
			title:"Query Result",
			param:paramGrid,
			colNames:["Order Differentiate","Document No.","Status","Flg","Store No.","Receiving Store","Submission Date","Delivery Date",
			...
			colModel:[
				...
				{name:"deliveryDate",type:"text",text:"center",width:"120",ishide:true,css:"",getCustomValue:dateFmt},
		...
		rrTableGrid = $("#rrGridTable").zgGrid({
			...
			param:paramGrid,
			localSort: false,
			colNames:["Status","Flg","Store No.","Receiving Store","Receiving Date","Delivery Date",
			...
			colModel:[
				...
				{name:"deliveryDate",type:"text",text:"center",width:"120",ishide:false,css:"",getCustomValue:dateFmt},
				{name:"receiveId",type:"text",text:"right",width:"140",ishide:false,css:""},
				{name:"orderId",type:"text",text:"right",width:"140",ishide:false,css:""},
				{name:"orderDate",type:"text",text:"center",width:"120",ishide:false,css:"",getCustomValue:dateFmt},
				{name:"reviewSts",type:"text",text:"left",width:"130",ishide:false,getCustomValue:getStatus},
