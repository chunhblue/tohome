...
define('returnVendorEdit', function () {
    var self = {};
    var url_left = "",
        systemPath = "",
        paramGrid = null,
        ...
        isFreeItem = null,//选中的商品是否为搭赠
        isFreeItemText = null,//选中的商品是否为搭赠
    ...
    var initAutoMatic = function () {
                    ...
                    $.myAjaxs({
                        ...
                        success: function (result) {
                            if (result.success) {
                                ...
                                m.vendor_id.val(isEmpty(data.vendorId));
                                m.vendor_name_in.val(isEmpty(data.vendorId + " " + data.vendorName));
                                auditNum = data.auditNum;
                                // returnFlg = data.thisReturn;
                            } else {
                                m.store_cd.val('');
                                m.store_name.val('');
                                m.vendor_id.val('');
        ...
        // 	selectEleClick: function (thisObj) {}
        // });
        // a_warehouse = $("#warehouseCd").myAutomatic({
        // 	url: systemPath + "/vendorMaster/get",
        // 	ePageSize: 8,
        // 	startCount: 0
        // });
        ...
        $.myAjaxs({
            ...
            success: function (result) {
                if (result.success && result.flag == "2") {
                    ...
                    m.receiveNochargeQty.val((toThousands(data.receiveNochargeQty)));
                    m.realtimeQty.val(data.realtimeQty);//实时在库量
                    m.receiveNochargeQty.val(data.receiveNochargeQty);
                    m.isFreeItem.val(data.isFreeItem);
                    m.isFreeItemText.val(itemType(data.isFreeItem));
        ...
        $.myAjaxs({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    m.vendor_id.val(isEmpty(result.data.vendorId));
                    m.vendor_name_in.val(result.data.vendorId + " " +isEmpty(result.data.vendorName));
                    ...
                    if(result.data.vendorId !== "" && result.data.vendorId != null){
                        $.myAutomatic.setValueTemp(warehouseCd, result.data.vendorId, result.data.vendorId + " " + result.data.vendorName);
                    }else {
                    ...
                    if (result.data.returnType == "10" && m.flag.val()=="1") {
                        // m.reType.val(result.data.returnType);
                        $("#itemDivOrg").hide();
                        $("#itemDivDirect").show();
                    } else if (result.data.returnType == "20" && m.flag.val()=="1") {
                        // m.reType.val(result.data.returnType);
                        //202010/28
                        $("#itemDivOrg").show();
                        $("#itemDivDirect").hide();
    ...
    var initPageByType = function () {
        ...
        if (m.flag.val() === '2') { //Add
            ...
            if (m.reType.val() === "20") {
                $("#org_order_id").prop('disabled', false);
                $("#org_order_id_refresh").show();
                $("#org_order_id_clear").show();
                $("#directDC").hide();
                $("#indirectReturn").show();
                ...
                $("#indirectReturn").hide();
                $("#aStore").prop('disabled', false);
                $("#warehouseCd").prop('disabled', false);
            }
            disableNotModify();
            //禁用审核按钮
        ...
        if (m.flag.val() === '1') {//Modify
            ...
            if (m.reType.val() === "20") {
                ...
                $("#warehouseCd").prop('disabled', true);
                $("#storeRefresh").hide();
                $("#storeRemove").hide();
    ...
    var judgement = function () {
        ...
        //供应商失效为空
        var vendorName = m.vendor_name_in.val();
        if (vendorName == null || vendorName.trim() === "" ) {
            _common.prompt("Please enter the correct Vendor No.!", 3, "info");
            m.org_order_id.focus();
            $("#org_order_id").css("border-color", "red");
            return false;
        } else if(vendorName.trim() === m.vendor_id.val()){
            _common.prompt("The vendor is no longer valid!", 3, "info");
            m.org_order_id.focus();
            m.org_order_id.css("border-color", "red");
            return false;
        } else {
            $("#org_order_id").css("border-color", "#CCC");
        }
    }
    ...
    var table_event = function () {
       ...
       $("#print").on("click",function () {
        ...
        m.returnsSubmitBut.on("click", function () {
            ...
            // if(warehouseCd==null||warehouseCd==""){
            // 	_common.prompt("Please select DC!",5,"error"); // 请选择DC
            // 	$("#warehouseCd").focus();
            // 	return false;
            // }
            ...
            let vendorId,orderRemarkflg = null;
            if (m.reType.val() == "10") {
                vendorId = $("#warehouseCd").attr('k');
                orderRemarkflg = $("#order_remark")[0].value;
            } else if (m.reType.val() == "20") {
                vendorId = m.vendor_id.val();
                orderRemarkflg = $("#order_remark_in")[0].value;
            }
            ...
            var returnOrder = {
                ...
                deliveryCenterId: "00000",//物流中心Id 无关紧要
            ...
            _common.myConfirm("Are you sure you want to save it？", function (result) {
                if (result != "true") {
                    return false;
                }
                $.myAjaxs({
        ...
        m.submit.on("click", function () {
            ...
            if (!isNaN(orderNochargeQty)) {
                if (orderNochargeQty > receiveNochargeQty) {//不得高于可退货数量
                    $("#orderNochargeQty").css("border-color", "red");
                    _common.prompt("Return Free Qty can not be greater than Order Free Qty!", 3, "info");
            ...
            if (m.reType.val() == "10") {
                if (flg == 'add') {
                    $("#zgGridTtable>.zgGrid-tbody tr").each(function () {
                        let _articleId = $(this).find('td[tag=articleId]').text();
                        if (_articleId == $("#a_item_direct").attr("k")) {
                            appendFlg = false;
                        }
                    });
                } else if (flg == 'update') {
    ...
    var itemClear = function () {
        ...
        $("#a_item_direct").val('').attr("k", '').attr("v", '');
        m.barcode.val('');
        m.unitId.val('');
        m.unitName.val('');
        m.spec.val('');
    ...
    var but_event = function () {
        ...
        $("#orderQty").blur(function () {
            var reg = /((^[1-9]\d*)|^0)(\.\d+)?$/;
            if (reg.test($("#orderQty").val()) && $("#orderQty").val().indexOf(",")<1){
                var orderQty = reThousands(m.orderQty.val());
                var price = reThousands(m.orderPrice.val());
                if (!!orderQty) {
                    m.orderAmount.val(parseFloat(orderQty * price).toFixed(2));//保留两位小数
...
var _index = require('returnVendorEdit');
