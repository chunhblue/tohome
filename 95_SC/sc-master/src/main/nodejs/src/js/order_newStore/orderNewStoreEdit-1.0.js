...
define('orderEdit', function () {
	...
	var approval_event = function () {
		...
		m.approvalBut.on("click",function(){
			var recordId = m.storeCd.val()+m.orderDate.val();
	...
	var table_event = function(){
		...
		// $("#update").on("click", function () {
		// 	$('#i_articleId').attr("disabled", "disabled");
		// 	$('#i_articleName').attr("disabled", "disabled");
		// 	$('#i_realtimeStock').attr("disabled", "disabled");
		// 	$('#i_psd').attr("disabled", "disabled");
		// 	$('#i_autoOrderQty').attr("disabled", "disabled");
		// 	if(selectTrTemp==null){
		// 		_common.prompt("Please select at least one item!",5,"error"); // 请选择要订货的商品
		// 		return false;
		// 	}
		...
		// 	}
		// 	var cols = tableGrid.getSelectColValue(selectTrTemp,"articleId,articleName,realtimeStock,psd,autoOrderQty,orderQty,vendorId,purchaseUnitId,orderPrice,dcItem");
		// 	$('#i_articleId').val(cols["articleId"]);
		// 	$('#i_articleName').val(cols["articleName"]);
		// 	$('#i_realtimeStock').val(cols["realtimeStock"]);
		// 	$('#i_psd').val(cols["psd"]);
		// 	$('#i_autoOrderQty').val(cols["autoOrderQty"]);
		// 	$('#i_orderQty').val(cols["orderQty"]);
		// 	$('#i_minOrderQty').val(toThousands(minOrderQty));
		// 	$('#i_maxOrderQty').val(toThousands(maxOrderQty));
		// 	$('#i_orderBatchQty').val(toThousands(orderBatchQty));
		// 	$('#orderAdd_dialog').modal("show");
		// });
		...
		m.returnsViewBut.on("click",function(){
			...
			top.location = url_left+"?"+url_param;
		});
	...
	var checkOrderQty = function () {
		...
		if(_orderQty != 0 && reThousands($("#orderBatchQty").val())!==0){
			var remainder = (_orderQty-_minOrderQty)%_orderBatchQty;
			if(remainder!==0){
				_common.prompt("Order Qty need to be multiples of Incremental Quantity!",3,"info");
				$('#i_orderQty').focus();
