...
define('receiptEdit', function () {
	...
	var attachments_event = function () {
		...
		$("#attachments_dialog").on("click","a[class*='downLoad']",function(){
			...
			window.open(encodeURI(url),"_self","width=1400,height=600,scrollbars=yes");
	...
	var table_event = function(){
		...
		$("#view").on("click", function (e) {
			...
			$("#update_affirm").prop("disabled", true);
		});
		...
		$("#update").on("click", function (e) {
			...
			var cols = tableGrid.getSelectColValue(selectTrTemp,"barcode,articleId,articleName,orderUnit,receivePrice,orderQty,orderNoChargeQty,receiveQty,receiveNoChargeQty,receiveTotalAmt,differenceReasonText");
			$("#barcode").val(cols['barcode']);
			$("#itemId").val(cols['articleId']);
			$("#itemName").val(cols['articleName']);
			$("#uom").val(cols['orderUnit']);
		...
		m.saveBtn.on("click",function(){
			...
			if(verifySearch()){
				...
				$("#zgGridTable>.zgGrid-tbody tr:not(:last-child)").each(function () {
					...
					if (_qty==""){
						_qty = null;
						// _qty = reThousands($(this).find('td[tag=receiveQty]').text());
					}
					...
					if (_receiveNoChargeQty==""){
						...
						// _receiveNoChargeQty=reThousands($(this).find('td[tag=receiveNoChargeQty]').text());
					}
					let _receiveTotalQty=reThousands($(this).find('td[tag=receiveTotalQty]').text());
					if(_receiveTotalQty==""){
						_receiveTotalQty = null;
						// _receiveTotalQty=reThousands($(this).find('td[tag=receiveTotalQty]').text());
					}
					let _taxRate = null,_amtNoTax = null,_amt = null,_tax = null;
					if(_qty != null && _price != null){
						//税率
						_taxRate = $(this).find('td[tag=taxRate]').text();
						_amtNoTax = Number(accMul(_qty, _price)).toFixed(2);
						_amt = Number(accMul(_amtNoTax, accAdd(1,_taxRate))).toFixed(2);
						_tax = Number(accMul(_amtNoTax, _taxRate)).toFixed(2);
					}
					let orderItem = {
						storeCd:_storeCd,
						orderId:_voucherNo,
					...
					$.myAjaxs({
						...
						success:function(result){
							if(result.success){
								...
								setDisable();
								tableGrid.find("tr").unbind("dblclick"); //移除双击事件
							}else{
	...
	var  verifyTable=function () {
		if($("#zgGridTable>.zgGrid-tbody tr").length>0){
			...
			$("#zgGridTable>.zgGrid-tbody tr:not(:last-child)").each(function () {
				...
				if (td_orderQty != td_receiveQty) {
					if (($(this).find('td[tag=differenceReasonText]').text()).trim() === "" || $(this).find('td[tag=differenceReasonText]').text() == null) {
						//添加input框
			...
			if (blankReason!=0){
				_common.prompt("Differnece Reason cannot be empty at the red mark!"); // 请输入店铺
				return false;
			}
			return true;
		}
	...
	var verifyDialogSearch = function(flag){
		...
		if(!checkNum(inputVal)){
			m.receiveQty.focus();
			m.updateAmount.val("0");
			_common.prompt("Received Quantity can only be a Positive number!",5,"error");
			return false;
		}
	...
	var initTable = function(){
		tableGrid = $("#zgGridTable").zgGrid({
			title:"Vendor Delivery Receiving  Document",
			param:paramGrid,
			localSort: true,
			height: "300",
	...
	var total_qty = function () {
		if($("#zgGridTable>.zgGrid-tbody tr").length>0){
			var orderQty=0,orderNoChargeQty=0,orderTotalQty=0,receiveQty=0,receiveNoChargeQty=0,receiveTotalQty=0,hhtReceiveQty=0;
			$("#zgGridTable>.zgGrid-tbody tr").each(function () {
				var td_orderQty= parseFloat($(this).find('td[tag=orderQty]').text().replace(/,/g, ""));
