...
define('order', function () {
	...
	var table_event = function(){
		...
		$("#update").on("click", function (e) {
			...
			getRecordStatus($("#aStore").attr("k")+subfmtDate(m.order_date.val()),m.typeId.val(),$("#aStore").attr("k"),function (result) {
				if(result.success||result.data=='0'){
					saveParamToSession();
					top.location = order_url+"/edit?"+param;
				}else{
	...
	var insertCopy = function () {
		$.myAjaxs({
			url: order_url + "/insertQuickCopy",
			async: true,
			cache: false,
			type: "post",
			data: {
			...
			error: function () {
				_common.prompt("Transfer data error!",5,"error");
			}
		})
	};

	// 判断该店铺今日是否有审核记录
	var isRecordStatus = function () {
		var recordCd = m.aStore.attr('k') + subfmtDate(m.order_date.val());
		var typeId = m.typeId.val();
		var nReviewid = m.reviewId.val();
		...
		$.myAjaxs({
			url:order_url+"/getStoreRecordStatus",
			async:true,
			cache:false,
			type :"post",
			data :{
	...
	var but_event = function(){
		...
		m.search.on("click",function(){
			if(verifySearch()){
				...
				tableGrid.setting("url",order_url+"/getList");
